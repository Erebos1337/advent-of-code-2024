...
fn run_program_2(registers: &[u64; 3], instructions: &Box<[u8]>, match_length: usize) -> Box<[u8]> {
    // Combo operands 0 through 3 represent literal values 0 through 3.
    // Combo operand 4 represents the value of register A.
    // Combo operand 5 represents the value of register B.
    // Combo operand 6 represents the value of register C.
    ...
    while ptr < instructions.len() {
        match instructions[ptr] {
            ...
            // (If a program outputs multiple values, they are separated by commas.)
            5 => {
                let out = (combo(&&registers, instructions[ptr + 1]) % 8) as u8;
                output.push(out);
                ptr += 2;
                if out != instructions[output.len() - 1] || output.len() >= match_length {
                    break;
                }
            }
            // The bdv instruction (opcode 6) works exactly like the adv instruction except that the result is stored in the B register.
    ...
    for i in 0..16 {
        ...
        for suffix in &suffixes {
            for j in 0..(1 << 10) {
                ...
                let output = run_program_2(&new_registers, instructions, match_length);
...
fn main() {
    ...
    let (solution1, duration1) = solve1(&registers, &instructions);
    let (solution2, duration2) = solve2(&registers, &instructions);

    println!("day 17");
    println!("  - part 1: {} \t{:?}", solution1, duration1); // 2,7,4,7,2,1,7,5,1
    println!("  - part 2: {} \t{:?}", solution2, duration2); // 37221274271220
}
